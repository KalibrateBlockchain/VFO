...
def run_analysis(wav_chunk, sampling_rate):
    ...
    g = glottal_flow_extractor(wav_chunk, sampling_rate, None, section = -1)
    results = vocal_fold_estimator(wav_chunk, sampling_rate, g, logging, t_patience = 100, section = -1)
    ...
    Sr, Sl = plot_phasor(wav_chunk, results, "")
...
def run_export_analysis(wav_file_path, window_size, overlap):
    ...
    for start_index in range(0, len(wav_samples)-window_size, overlap):
        ...
        results = run_analysis(wav_chunk, sampling_rate)
