...
def vfo_vocal_fold_estimator(glottal_flow,wav_samples,sample_rate):
    ...
    while patience < t_patience: # this was 400 default
        ...
        if min_distance>=1 and Rk_s<=1 and d_1/g_1>=.6: #We have a solution that meets threshold criteria
...
def CWWmain(fname, mode_of_processing):
  ...
  while run<1:
    ...
    if res['min_distance']>=1.0 and res['Rk_s']<=1.0 and res['distanceRatio']>0.6:
      run=6
