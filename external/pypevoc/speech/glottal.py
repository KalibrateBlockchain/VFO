...
def iaif_ola(
    ...
    tract_order=None,
    glottal_order=None,
    leaky_integration=0.99,
    wind_func=np.hanning,
    n_it=1,
):
...
class PaddedFilter(object):
    def __init__(self, input_signal, n_before=0, n_after=0, mode="zeros"):
...
class InverseFilter(object):
    ...
    def apply(self, x, n_it=1):
        ...
        for ii in range(self.hpfilt):
            ...
            y = y[hp_pad:]
        # create a padded filter object for chained filtering
        filter_machine = PaddedFilter(n_before=self.n_pad, input_signal=y, mode="ramp")
        ...
        for ii in range(n_it):
            ...
            g = filter_machine.apply_filter_to_last_output(self.id, self.leaky_integrator)

            Hg = lpc(g * self.wind, self.glottal_order)
            y = filter_machine.apply_filter(Hg, self.id)
            y = filter_machine.apply_filter_to_last_output(self.id, self.leaky_integrator)
