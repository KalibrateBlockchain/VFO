...
def toy_probelm_1():
    ...
    c = 1.0  # speed of sound in medium
    alpha = 1.0
    beta = 5.0
    ...
    # Initial expression
    u_I = F.Expression(g_expr, alpha=alpha, beta=beta, t=0, degree=BASIS_DEGREE + 2)

    # Exact solution
    u_e = F.Expression(g_expr, alpha=alpha, beta=beta, t=0, degree=BASIS_DEGREE + 3)
    ...
    def boundary(x, on_boundary):
        ...
        """
        tol = 1e-14
        return on_boundary and ((F.near(x[0], 0.0, tol)) or (F.near(x[0], length, tol)))
...
def toy_probelm_2():
    ...
    u_D = F.Expression(g_expr, alpha=alpha, beta=beta, t=0, degree=BASIS_DEGREE + 2)
